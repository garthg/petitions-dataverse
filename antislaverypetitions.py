...
def ParseAntislaveryPetitions(input_rows):
  ...
  for row in input_rows:
    ...
    for date_field in date_fields:
      if curr.Has(row, date_field):
        try:
          row[date_field] = dataversestudybuilder.ParseDate(row[date_field])
        except ValueError, e:
          print 'WARNING: Failed to parse date, will treat as string:\n%s' % (
              traceback.format_exc(e))

    # Fields
    curr.Set('Author', 'Digital Archive of Massachusetts Anti-Slavery and Anti-Segregation Petitions')
    ...
    curr.Set('Country/Nation', 'United States of America')
    ...
    for field in ['dateaction%d' % i for i in range(1,7)]:
      if curr.Has(row, field):
        ...
        try:
          action_dates.append(dataversestudybuilder.ParseDate(row[field]))
        except ValueError, e:
          print 'WARNING: Failed to parse date, will treat as string:\n%s' % (
              traceback.format_exc(e))
    if action_dates:
    ...
    curr.AddDescriptionEntry(row, 'Subject', 'Petition', 1)
    ...
    if curr.Has(row, 'Identifications'):
      ...
      for ident in identifications:
        ...
        for charpair in ('[]', '()', '""'):
          if ident.startswith(charpair[0]) and ident.endswith(charpair[1]):
    ...
    if curr.Has(row, 'At least 3 signatures from the petition'):
      ...
      if signatures:
        ...
        curr.AddDescriptionHtml('<p>Top signatures:<ol><li>%s</li></ol> </p>' %
            '</li><li>'.join(signatures), 6)
    title_parts = []
    if curr.Has(row, 'Scholarly citation'):
      curr.used_input_columns.add('Scholarly citation')
      citation = row['Scholarly citation']
      result = re.match(r'.*Petitions(.*) Massachusetts Archives',
          citation)
      if result:
    ...
    if separation_bool is not None:
      ...
      curr.AddDescriptionHtml('<p>Signatory column format: %s </p>' % value, 19)
...
def RunMain(input_tsv,
  ...
  output_studies = ParseAntislaveryPetitions(input_rows)
